	...
	useEffect(() => {
	  const fetchUserData = async () => {
		try {
		  ...
		  const response = await axios.get('https://bofa-cap2.onrender.com/getuserinfo', {
...
const Survey = () => {
	...
	const handleSubmit = async (e) => {
	  ...
	  try {
		...
		await axios.post('https://bofa-cap2.onrender.com/api/survey', responses);
	...
	useEffect(() => {
		const checkSurveyCompletion = async () => {
		  try {
			const response = await axios.get('https://bofa-cap2.onrender.com/api/survey/completed');
